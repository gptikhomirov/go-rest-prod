...
type PatchUserRequest struct {
	FullName    core_http_types.Nullable[string] `json:"full_name"    swaggertype:"string"                         example:"Ivanov Ivan"`
	PhoneNumber core_http_types.Nullable[string] `json:"phone_number" swaggertype:"string" extensions:"x-nullable" example:"+79998887766"`
}
...
func (r *PatchUserRequest) Validate() error {
	if r.FullName.Set {
		...
		if fullNameLen < 3 || fullNameLen > 100 {
			return fmt.Errorf("`FullName` name must be between 3 and 100 symbols")
		}
	}
	...
	if r.PhoneNumber.Set {
		if r.PhoneNumber.Value != nil {
			...
			if phoneNumberLen < 10 || phoneNumberLen > 15 {
				return fmt.Errorf("`PhoneNumber` name must be between 10 and 15 symbols")
			}
...
// @Description 2. **Явно передано значение**: `"phone_number": "+711122233344"` — устанавливает новый номер
...
func (h *UsersHTTPHandler) PatchUser(rw http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		responseHandler.ErrorResponse(err, "failed to get userID from url")
